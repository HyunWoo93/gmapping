...
class SlamGMapping
{
  public:
	...
	// method 
    void startLiveSlam(); // start SLAM
    void startReplay(const std::string & bag_fname, std::string scan_topic);
 
	// callback function
    void laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan); // core
  ...
  private:
	// called in laser callback
	void updateMap(const sensor_msgs::LaserScan& scan);
    bool getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t);
    bool initMapper(const sensor_msgs::LaserScan& scan); // init gsp
    bool addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose); // do gsp
    double computePoseEntropy();

	// for publishing
    ros::NodeHandle node_;
    ros::Publisher entropy_publisher_;	// entropy : means unknown info.
    ros::Publisher sst_;	// map
    ros::Publisher sstm_;	// map metadata
    ros::ServiceServer ss_;	// dynamic_map service -> map file
	...
	// ?
    double occ_thresh_; // occupancy threshhold
