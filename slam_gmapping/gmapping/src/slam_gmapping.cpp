...
void SlamGMapping::startLiveSlam()
{
  ...
  // A thread publishing map_to_odom tf.
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  // convert to GMapping reading stucture
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if(addScan(*scan, odom_pose))
  {
	...
	// select mpose : which is new robot pose
    GMapping::OrientedPoint mpose = gsp_->getParticles()[gsp_->getBestParticleIndex()].pose;
    ROS_DEBUG("new best pose: %.3f %.3f %.3f", mpose.x, mpose.y, mpose.theta);
    ROS_DEBUG("odom pose: %.3f %.3f %.3f", odom_pose.x, odom_pose.y, odom_pose.theta);
    ROS_DEBUG("correction: %.3f %.3f %.3f", mpose.x - odom_pose.x, mpose.y - odom_pose.y, mpose.theta - odom_pose.theta);

	// calculate laser_to_map
    tf::Transform laser_to_map = tf::Transform(tf::createQuaternionFromRPY(0, 0, mpose.theta), tf::Vector3(mpose.x, mpose.y, 0.0)).inverse();
	// calculate odom_to_laser
    tf::Transform odom_to_laser = tf::Transform(tf::createQuaternionFromRPY(0, 0, odom_pose.theta), tf::Vector3(odom_pose.x, odom_pose.y, 0.0));

	// update map_to_odom
    map_to_odom_mutex_.lock();
    map_to_odom_ = (odom_to_laser * laser_to_map).inverse(); // odom <- laser <- map 
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  boost::mutex::scoped_lock map_lock (map_mutex_); // mutex
  ...
  // create smap using best particle's trajectory
